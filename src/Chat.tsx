...
export default function Chat() {
  ...
  const handleVideoCall = () => {
    if (!chatId) return;
    const roomName = `SkillSwap_${chatId}`;
    const jitsiUrl = `https://meet.jit.si/${roomName}`;
    
    // Send a message with the link
    const messagesRef = collection(db, 'chats', chatId, 'messages');
    addDoc(messagesRef, {
      text: `🎥 Join my video call: ${jitsiUrl}`,
      senderId: user?.uid,
      timestamp: Date.now()
    });
  ...
  return (
    <div className="max-w-4xl mx-auto h-[calc(100vh-12rem)] flex flex-col bg-white/[0.03] border border-white/5 rounded-[2.5rem] overflow-hidden">
      {/* Chat Header */}
      <div className="p-6 border-b border-white/10 flex items-center justify-between bg-[#0a0a0a]">
        ...
        <button 
          ...
          className="flex items-center gap-2 bg-emerald-500/10 hover:bg-emerald-500/20 text-emerald-400 px-4 py-2 rounded-full text-sm font-medium transition-colors cursor-pointer"
        >
